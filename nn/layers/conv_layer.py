...
class ConvNumbaLayer(Layer):
    ...
    def forward_numba(data, weights, bias, stride, kernel_size):
        f = kernel_size
        s = stride
            ...
            for c in prange(ochn):
                for w in prange(w_n):
                    for h in prange(h_n):
                        ...
                        s = 0
                        for inputchn in range(0,ichn):
                            a_prev_slice = data[i, inputchn, horiz_start:horiz_end, vert_start:vert_end]
                            convkernel = weights[inputchn][c]
                            val = np.sum(a_prev_slice * convkernel)
                            s += val
                        A[i, c, w, h] = s + bias[c]
        return A
    ...
    def backward_numba(previous_partial_gradient, padded_data, weights, stride,f,data,padding):

        smp, chnls, hei, wid = previous_partial_gradient.shape
        ichn, ochn, k1, k2 = weights.shape
        dW = np.zeros((ichn, ochn, k1, k2))
        data_size, c_n, w_prev, h_prev = padded_data.shape
        dX = np.zeros((data_size, c_n, w_prev, h_prev))
            ...
            for c in prange(ochn):
                for h in range(0, hei):
                    for w in range(0, wid):
                        ...
                        horiz_end = w * stride + f
                        kernel = weights[:,c]
                        gprev = previous_partial_gradient[i, c, h, w]
                        dX[i, :, vert_start:vert_end, horiz_start:horiz_end] += kernel * gprev
        if dX.shape != data.shape:
            dX = dX[:, :, padding:dX.shape[2] - padding, padding:dX.shape[3] - padding]

        strided_data = np.zeros((smp, hei, wid))
        for ci in range(ichn):
            for co in range(ochn):
                for h in range(k1):
                    for w in range(k2):
                        for part in range(len(padded_data)):
                            strided_data[part,...] = padded_data[part,ci,h::stride, w::stride][:hei, :wid]
                        dW[ci, co, h, w] += np.sum(strided_data * previous_partial_gradient[:, co, :, :])
        return dX, dW
    ...
    def backward(self, previous_partial_gradient):
        ...
        db = np.sum(previous_partial_gradient, axis=(0, 2, 3))
        self.bias.grad = db
        dX, dW = self.backward_numba(previous_partial_gradient, self.input, weights, self.stride,self.kernel_size, self.data,self.padding)
        self.weight.grad = dW
        return dX
